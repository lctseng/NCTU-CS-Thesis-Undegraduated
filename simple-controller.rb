...
UPSTREAM_INFO.each_pair do |port,list|
    data = {port: port,connect: false,avg_speed: 0.0,host_count: 0,div_spd: MAX_SPEED_M,should_spd: MAX_SPEED_M,last_should_spd: MAX_SPEED_M,new_should_spd: MAX_SPEED_M}
    list.each do |host|
    ...
    UPSTREAM_INFO[id].each do |host_id|
        ...
        if host_data
            ...
            if mul
                ...
                if $host_belong_sw[host_id].any? {|data| (curr_spd - data[:avg_speed]) > data[:avg_speed] * 0.1}
                    final_add -= 100
                end
...
def show_info
    ...
    $client_sw.clone.each_pair do |id,data|
        ...
        printf("%8s,限：%3d M, 均：%8.3f M,商：%3d M,應：%3d M,%5d,%s\n",id,data[:spd],$port_data[id][:avg_speed] / 1000000.0,$port_data[id][:div_spd],$port_data[id][:should_spd],len,"|"*bar_len)
    end
    puts "===各host speed狀況==="
